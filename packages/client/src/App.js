...
function AppContent() {
    ...
    return (_jsxs("div", { className: "flex flex-col h-full w-full bg-[#0d0b16]", children: [_jsxs("nav", { className: "flex items-stretch bg-[#14112a] border-b border-[#2e2848] flex-shrink-0 px-4 gap-1", children: [_jsx("button", { onClick: handleBackToProjects, className: "flex items-center gap-1.5 pr-4 mr-2 border-r border-[#2e2848] text-xs text-[#5c5575] hover:text-[#9b93ba] transition-colors", title: "All projects", children: "\u2190" }), _jsx("button", { onClick: handleBackToLanding, className: "flex items-center pr-4 mr-2 border-r border-[#2e2848]", title: "Home", children: _jsx("img", { src: "/HERO.png", alt: "Vigo", className: "h-9 w-9 object-contain rounded-lg" }) }), TABS.map(({ phase, label, sub }) => (_jsxs("button", { onClick: () => setActivePhase(phase), className: `flex flex-col items-start justify-center px-4 py-2.5 text-left border-b-2 transition-colors ${activePhase === phase
                            ? "border-[#8b5cf6] text-[#a78bfa]"
                            : "border-transparent text-[#5c5575] hover:text-[#9b93ba] hover:border-[#3d366a]"}`, children: [_jsx("span", { className: "text-xs font-semibold uppercase tracking-wide leading-none", children: label }), _jsx("span", { className: "text-sm font-medium leading-tight mt-0.5", children: sub })] }, phase))), _jsxs("div", { className: "flex items-center ml-auto gap-3 pr-2", children: [activeProject && (_jsx("span", { className: "text-xs text-[#5c5575] truncate max-w-48", title: activeProject.name, children: activeProject.name })), activePhase === 2 && _jsx(ExportMenu, {}), _jsx(JiraMenu, {}), _jsx("button", { onClick: handleSave, disabled: saveState === "saving", className: "flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold transition-all disabled:opacity-60", style: saveState === "saved"
                                    ? { background: "rgba(34,197,94,0.15)", border: "1px solid rgba(34,197,94,0.4)", color: "#86efac" }
                                    : { background: "rgba(124,58,237,0.15)", border: "1px solid rgba(124,58,237,0.4)", color: "#a78bfa" }, children: saveState === "saving" ? "Saving…" : saveState === "saved" ? "Saved ✓" : "Save" })] })] }), _jsxs("div", { className: "flex-1 min-h-0 overflow-hidden", children: [activePhase === 1 && _jsx(EstimationsPage, {}), activePhase === 2 && _jsx(SchedulingPage, {}), activePhase === 3 && _jsx(MilestonesPage, {}), activePhase === 4 && _jsx(CostSheetPage, {})] })] }));
}
export function App() {
