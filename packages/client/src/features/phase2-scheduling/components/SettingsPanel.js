...
export function SettingsPanel({ settings, onChange }) {
    ...
    const [budgetDraft, setBudgetDraft] = useState(settings.revenueGBP > 0 ? String(toDisplay(settings.revenueGBP)) : "");
    useEffect(() => {
        setBudgetDraft(settings.revenueGBP > 0 ? String(Math.round(settings.revenueGBP * conversionRate)) : "");
    ...
    function commitMonthRate(raw) {
        ...
        onChange({ defaultMonthlyRate: monthly / conversionRate });
        setMonthDraft(monthly > 0 ? String(Math.round(monthly)) : "");
    }
    function commitBudget(raw) {
        const v = parseFloat(raw);
        const display = isNaN(v) || v < 0 ? 0 : v;
        onChange({ revenueGBP: display / conversionRate });
        setBudgetDraft(display > 0 ? String(Math.round(display)) : "");
    }
    return (_jsxs("div", { className: "flex items-end gap-6 px-6 py-3 bg-[#14112a] border-b border-[#2e2848] flex-shrink-0 flex-wrap", children: [_jsx(Field, { label: "Project name", children: _jsx("input", { className: "border border-[#2e2848] bg-[#1a1628] text-[#ece7ff] rounded-lg px-3 py-1.5 text-sm w-52 focus:outline-none focus:ring-2 focus:ring-[#7c3aed] placeholder:text-[#3a3456]", value: settings.projectName, onChange: (e) => onChange({ projectName: e.target.value }) }) }), _jsx(Field, { label: "Calendar mode", children: _jsx("div", { className: "flex rounded-lg border border-[#2e2848] overflow-hidden text-sm", children: ["four-week", "actual"].map((mode) => (_jsx("button", { className: `px-3 py-1.5 transition-colors ${settings.calendarMode === mode
                        ...
                        e.currentTarget.blur(); } }) }), _jsx(Field, { label: `Budget / project value (${symbol})`, children: _jsxs("div", { className: "flex items-center gap-1", children: [_jsx("span", { className: "text-sm text-[#5c5575]", children: symbol }), _jsx("input", { type: "number", min: 0, step: 1000, placeholder: "e.g. 150000", className: "border border-[#2e2848] bg-[#1a1628] text-[#ece7ff] rounded-lg px-2 py-1.5 text-sm w-32 focus:outline-none focus:ring-2 focus:ring-[#7c3aed] placeholder:text-[#3a3456]", value: budgetDraft, onChange: (e) => setBudgetDraft(e.target.value), onBlur: () => commitBudget(budgetDraft), onKeyDown: (e) => { if (e.key === "Enter")
                                e.currentTarget.blur(); } })] }) }), _jsx(Field, { label: "Agency fee", children: _jsxs("div", { className: "flex items-center gap-1.5", children: [_jsx("input", { type: "number", min: 0, max: 100, step: 1, className: "border border-[#2e2848] bg-[#1a1628] text-[#ece7ff] rounded-lg px-3 py-1.5 text-sm w-16 focus:outline-none focus:ring-2 focus:ring-[#7c3aed]", value: settings.agencyFeePct ?? 0, onChange: (e) => onChange({ agencyFeePct: Math.max(0, Math.min(100, Number(e.target.value))) }) }), _jsx("span", { className: "text-sm text-[#5c5575]", children: "%" }), _jsx("input", { className: "border border-[#2e2848] bg-[#1a1628] text-[#ece7ff] rounded-lg px-2 py-1.5 text-sm w-20 focus:outline-none focus:ring-2 focus:ring-[#7c3aed] placeholder:text-[#3a3456]", placeholder: "Label", value: settings.agencyFeeLabel ?? "", onChange: (e) => onChange({ agencyFeeLabel: e.target.value }) })] }) })] }));
}
