import type { ScheduledTask, ScheduleResult } from "./scheduler.js";
...
export function exportTimelinePng(projectName: string): void {
  const labelSvg = document.querySelector("[data-label-svg]") as SVGSVGElement | null;
  const chartSvg = document.querySelector("[data-chart-svg]") as SVGSVGElement | null;
  if (!labelSvg || !chartSvg) return;
  ...
  img.onload = () => {
    ...
    canvas.toBlob((pngBlob) => {
      if (pngBlob) downloadBlob(pngBlob, safeFilename(projectName, "timeline", "png"));
    });
  };
...
export function exportCsv(tasks: ScheduledTask[], settings: ScheduleSettings): void {
  ...
  downloadBlob(
    ...
    safeFilename(settings.projectName, "schedule", "csv")
  );
}

// ── JSON ──────────────────────────────────────────────────────────────────────

export function exportJson(data: unknown, projectName: string): void {
  downloadBlob(
    new Blob([JSON.stringify(data, null, 2)], { type: "application/json" }),
    safeFilename(projectName, "schedule", "json")
  );
}
