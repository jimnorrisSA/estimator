...
function recorded(set, get, updater) {
...
export const useEstimationsStore = create()(persist((set, get) => ({
    features: [],
    selectedId: null,
    selectedIds: [],
    _past: [],
    ...
    deleteFeature(id) {
        ...
        if (get().selectedId) {
            const stillExists = get().features.some((f) => f.id === get().selectedId || f.groups.some((g) => g.tasks.some((t) => t.id === get().selectedId)));
            if (!stillExists)
                set({ selectedId: null });
        }
    ...
    updateFeatureName(id, name) {
        recorded(set, get, (fs) => fs.map((f) => (f.id === id ? { ...f, name, updatedAt: now() } : f)));
    },
    updateFeaturePosition(id, pos) {
        recorded(set, get, (fs) => fs.map((f) => (f.id === id ? { ...f, position: pos, updatedAt: now() } : f)));
    },
    batchUpdateFeaturePositions(updates) {
        const posMap = new Map(updates.map((u) => [u.id, u.pos]));
        recorded(set, get, (fs) => fs.map((f) => (posMap.has(f.id) ? { ...f, position: posMap.get(f.id), updatedAt: now() } : f)));
    },
    updateFeatureWidth(id, width) {
        recorded(set, get, (fs) => fs.map((f) => (f.id === id ? { ...f, width, updatedAt: now() } : f)));
    },
    addGroup(featureId, discipline) {
        recorded(set, get, (fs) => fs.map((f) => f.id !== featureId
            ? f
    ...
    addTask(featureId, groupId, label) {
        ...
        recorded(set, get, (fs) => fs.map((f) => f.id !== featureId
            ...
            : {
                ...f,
                groups: f.groups.map((g) => g.id !== groupId
                    ? g
                    : {
                        ...g,
                        tasks: [
        ...
        recorded(set, get, (fs) => fs.map((f) => f.id !== featureId
            ...
            : {
                ...
                groups: f.groups.map((g) => g.id !== groupId
                    ...
                    : {
                        ...g,
                        tasks: g.tasks.map((t) => t.id === taskId ? { ...t, estimate: { value, unit } } : t),
                        updatedAt: now(),
                    }),
            }));
    },
