...
export function DisciplineGroupCard({ group, layout, featureId, selectedId, onSelect, stageScale }) {
    ...
    return (_jsxs(Group, { ref: cardRef, x: layout.x, y: layout.y, children: [_jsx(Rect, { width: layout.width, height: layout.height, fill: group.color, cornerRadius: 4, shadowBlur: 4, shadowColor: "rgba(0,0,0,0.3)", shadowOffsetY: 2 }), _jsx(Rect, { width: layout.width, height: GROUP_HEADER_H, fill: headerColor, cornerRadius: [4, 4, 0, 0] }), _jsx(Text, { x: PAD, y: (GROUP_HEADER_H - HEADER_FONT) / 2, width: layout.width - PAD * 2, text: group.discipline.toUpperCase(), fontSize: HEADER_FONT, fontStyle: "bold", fill: "rgba(255,255,255,0.85)", letterSpacing: 0.8 }), group.tasks.map((task, i) => {
                ...
                return (_jsxs(Group, { y: ty, onMouseEnter: () => setHoveredTaskId(task.id), onMouseLeave: () => setHoveredTaskId(null), onClick: () => onSelect(task.id), onTap: () => onSelect(task.id), onDblClick: () => openTaskEdit(task.id, task.label, ty), onDblTap: () => openTaskEdit(task.id, task.label, ty), children: [isSelected && _jsx(Rect, { width: layout.width, height: TASK_ROW_H, fill: "rgba(255,255,255,0.25)" }), _jsx(Rect, { y: 0, width: layout.width, height: 1, fill: "rgba(0,0,0,0.15)" }), _jsx(Text, { x: PAD, y: (TASK_ROW_H - LABEL_FONT) / 2, width: layout.width - PAD * 2 - EST_HIT_W - ACTION_W, text: task.label || "Double-click to label…", fontSize: LABEL_FONT, fill: task.label ? "rgba(255,255,255,0.95)" : "rgba(255,255,255,0.35)", ellipsis: true }), _jsx(Text, { x: layout.width - EST_HIT_W, y: (TASK_ROW_H - EST_FONT) / 2, width: EST_HIT_W - PAD, text: estText, fontSize: EST_FONT, fontStyle: "italic", fill: isSelected ? "#f0d9ff" : "rgba(255,255,255,0.65)", align: "right" }), _jsx(Rect, { x: layout.width - EST_HIT_W, y: 0, width: EST_HIT_W, height: TASK_ROW_H, fill: "transparent", onDblClick: (e) => {
                                e.cancelBubble = true;
                                openEstimateEdit(task.id, task.estimate.value, task.estimate.unit, ty);
                            }, onDblTap: (e) => {
                                e.cancelBubble = true;
                                openEstimateEdit(task.id, task.estimate.value, task.estimate.unit, ty);
                            } }), isHovered && (_jsxs(_Fragment, { children: [_jsxs(Group, { x: layout.width - EST_HIT_W - ACTION_W, y: 0, onClick: (e) => { e.cancelBubble = true; duplicateTask(featureId, group.id, task.id); }, onTap: (e) => { e.cancelBubble = true; duplicateTask(featureId, group.id, task.id); }, children: [_jsx(Rect, { width: DUPE_W, height: TASK_ROW_H, fill: "rgba(255,255,255,0.12)", cornerRadius: 2 }), _jsx(Text, { x: 0, y: (TASK_ROW_H - 11) / 2, width: DUPE_W, text: "\u29C9", fontSize: 11, fill: "rgba(255,255,255,0.8)", align: "center" })] }), _jsxs(Group, { x: layout.width - EST_HIT_W - DEL_W, y: 0, onClick: (e) => { e.cancelBubble = true; deleteTask(featureId, group.id, task.id); }, onTap: (e) => { e.cancelBubble = true; deleteTask(featureId, group.id, task.id); }, children: [_jsx(Rect, { width: DEL_W, height: TASK_ROW_H, fill: "rgba(239,68,68,0.15)", cornerRadius: 2 }), _jsx(Text, { x: 0, y: (TASK_ROW_H - 11) / 2, width: DEL_W, text: "\u00D7", fontSize: 13, fill: "rgba(239,68,68,0.85)", align: "center" })] })] }))] }, task.id));
